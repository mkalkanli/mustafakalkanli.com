...
void test('Turkish dictionary builds a temporary one-screen personal entrypoint', async () => {
  ...
  assert.match(index, /<script src="script\.js\?v=5" defer><\/script>/);
  assert.match(index, /href="styles\.css\?v=5"/);
...
void test('temporary profile keeps the restrained responsive visual system', async () => {
  ...
  assert.match(index, /<details class="expertise-menu">[\s\S]*<summary>Menü<\/summary>/);
  assert.match(css, /\.expertise-menu\s*\{[^}]*top:\s*1\.5rem;[^}]*right:\s*0;/);
  assert.match(css, /grid-template-columns:\s*repeat\(3, auto\)/);
  assert.match(css, /\.expertise-menu summary\s*\{[^}]*display:\s*flex;/);
